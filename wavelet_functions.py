...
def wavelet(Y, dt, pad=0, dj=-1, s0 = -1, J1=-1, mother=-1, param=-1, mean = False):
	n1 = len(Y)
	...
	if pad == 1:
		...
		x = np.concatenate((x, np.zeros(int(2 ** (base2 + 1) - n1))))
		#x = np.concatenate((np.zeros(int(2 ** (base2 + 1) - n1)), x, np.zeros(int(2 ** (base2 + 1) - n1))))[:-1]

	n = len(x)

	#....construct angular frequency array used in transform [Eqn(5)]
